...
function OrderDetailPage() {
  ...
  return (
    <div data-testid="order-detail-page">
      ...
      {isRole("buyer") && order.status === "pending" && (
        <div className="mt-6 bg-white rounded-lg shadow-sm border border-gray-line p-6">
          <h3 className="font-semibold mb-4">Payment</h3>
          <p className="text-sm text-gray-txt mb-4">This order is pending payment. Proceed to checkout to complete your purchase.</p>
          <Link href={`/checkout/${id}`}>
            <Button testId="btn-go-to-checkout">Proceed to Checkout</Button>
          </Link>
        </div>
      )}
      ...
      {isRole("admin") && (
        <div className="mt-6 bg-white rounded-lg shadow-sm border border-gray-line p-6 space-y-4">
          ...
          <Button variant="danger" onClick={() => setShowDeleteModal(true)} testId="btn-delete-order">Delete Order</Button>
        </div>
      )}
